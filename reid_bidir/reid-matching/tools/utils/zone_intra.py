...
class zone():
    ...
    def is_ignore(self,zone_list,frame_list, cid):
        # 0 not at any intersection 1 white 2 red 3 green 4 blue
        zs, ze = zone_list[0], zone_list[-1]
        fs, fe = frame_list[0],frame_list[-1]
        if zs == ze:
            # If it is always in the same area, exclude
            if ze in [1,2]:
                return 2
            ...
            if cid in [41, 42, 43, 44, 45, 46]:
                # If entering and exiting from a branch road, exclude
                if zs == 1 and ze == 2:
                    return 2
                if zs == 2 and ze == 1:
                    return 2
            if cid in [41]:
                # At camera 41, the vehicle does not enter or exit camera 42
                if (zs in [1, 2]) and ze == 4:
                    return 2
                if zs == 4 and (ze in [1, 2]):
                    return 2
            if cid in [46]:
                # At camera 46, the vehicle does not enter or exit camera 45
